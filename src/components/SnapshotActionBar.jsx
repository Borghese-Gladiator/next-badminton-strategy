...
const SnapshotActionBar = () => {
  ...
  useEffect(() => {
    if (leftPress && snapshots.length != 0) {
      const newSelectedSnap =
        selectedSnap - 1 < 0 ? selectedSnap : selectedSnap - 1;
  ...
  useEffect(() => {
    if (rightPress && snapshots.length != 0) {
      const newSelectedSnap =
        selectedSnap + 1 > snapshots.length - 1
          ? selectedSnap
          : selectedSnap + 1;
      setSelectedSnap(newSelectedSnap);
      setLines(snapshots[newSelectedSnap]);
  ...
  const snapshotRevertList = useMemo(
    () =>
      ...
      {actions.map(({ text, func }, idx) => {
        return (
          <Button
            key={`${text}-${idx}`}
            variant={
              idx === selectedSnap + baseActions.length
                ? 'contained'
                : 'outlined'
            }
            onClick={() => func()}
